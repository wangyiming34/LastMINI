	  ...
	  <view class="" style="position: fixed;left: 0; top: 0;z-index: 100;width: 100%;">
				...
				<view class="headTab border-bottom-new2 flex justify-around">
					<view v-for="(item,index) in tabList" :key="index" class="headTabFor tabtitle" @click="tab(index)" :class="{tabActive:index==n}">
						{{index==0?item+'('+count+')':item+'('+count1+')'}}
					</view>
				</view>
				<!-- 搜索 -->
	  			<view class="report_search flex align-center border-bottom" style="background-color: white;width: 100%;top: 44px;">
	        ...
	        <view class="bnew2 flex justify-start lii" v-for="(item,index) in list" :key="index">
	          ...
	          <view class="con" style="display: block">
	            ...
	            <view class="detail">
				  ...
				  <view v-if="type==1">注册时间：{{item.created_at}}</view>
				  <view v-else>续费时间：{{item.xufei_time}}</view>
				  <!-- <view v-if="item.xufei_time.length&&type==2">续费时间：{{item.xufei_time}}</view> -->
				<!--  <view v-if="item.xufei_time.length==0&&type==2">续费时间：暂无</view> -->
...
<script>
	export default {
		...
		methods: {
			...
			getData1(){
				this.$api.month_open_renew_list({
