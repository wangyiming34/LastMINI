	...
	<view class="StretchIndexSend" style="padding-bottom: 65px;">
		<view class="StretchIndexSend-padding">
				  ...
				  <view  @click.stop="tabpick" class="border-bottom StretchIndexSend-content">
					  ...
					  <view class="flex" style="flex-wrap: wrap; margin-top: 5px;" v-if="plist.length>0">
					  	<view v-for="(item,index) in plist" :key="index" class="alltab1">
					  		<text>{{item}}</text>
					  	</view>
					  </view>
				  </view>
...
<script>
	...
	export default {
		...
		methods: {
			tabpick(){
				uni.navigateTo({
					url:'../../pagesMy/TabSelect/TabSelect?status=0'
				})
			},
			// 选择金额
			pickershop(e){
			...
			send () {
				  var that = this		      
			      if (this.name && this.plist.length > 0 && this.address && this.money && this.content && this.fileList.length > 0 ) {
			        Dialog.confirm({
			          title: '发布提示',
