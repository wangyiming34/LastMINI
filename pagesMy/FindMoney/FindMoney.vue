	...
	<view class="StretchIndexSend" style="padding-bottom: 65px;">
		<view class="StretchIndexSend-padding">
				  ...
				  <view  @click.stop="tabpick" class="border-bottom StretchIndexSend-content">
					  ...
					  <view class="flex" style="flex-wrap: wrap; margin-top: 5px;" v-if="mlist.length>0">
					  	<view v-for="(item,index) in mlist" :key="index" class="alltab1">
					  		<text>{{item}}</text>
					  	</view>
					  </view>
				  </view>
		          ...
		          <view class="StretchIndexSend-content flex justify-between align-center border-bottom re">
		              ...
		              <view class="StretchIndexSend-name">所需资金</view>
...
<script>
	...
	export default {
		...
		methods: {
			tabpick(){
				uni.navigateTo({
					url:'../../pagesMy/TabSelect/TabSelect?status=1'
				})
			},
			// 选择金额
			pickershop(e){
			...
			send () {
				  var that = this		      
			      if (this.name && this.mlist.length > 0 && this.address && this.money && this.content && this.fileList.length > 0 ) {
			        Dialog.confirm({
			          title: '发布提示',
